...
const addEmployeeData = async (employeeData: any) => {
  try {
    ...
    if (isEmailExists) {
      ...
      const getBranchDetails: any = await qGetBranchDataByName(
        employeeData.selectedBranch
      );
      const employeeId =
        getBranchDetails.branchCode + "00" + getBranchDetails.empCount;
      ...
      if (getBranchDetails && employeeId) {
        ...
        return {
          status: "FAILED",
          message: "Employee creation Failed due to branch error",
        };
      }
    }
  } catch (err) {
